...
{
	//if ((s > MAX_VECTOR_SIZE) || (s < 0)) throw invalid_argument("Not correct size");
	//if (s < 0) throw invalid_argument("N");
	//if (si < 0) throw invalid_argument("Not correct start index");
	if ((s > MAX_VECTOR_SIZE) || (s < 0)) throw invalid_argument("Not correct size");
	if ((si < 0)) throw invalid_argument("Not correct start index");
	Size = s;
	StartIndex = si;
...
{
	if (s > MAX_MATRIX_SIZE) throw invalid_argument("MATRIX OVERFLOW");
...
TMatrix<ValType> TMatrix<ValType>::operator+(const TMatrix<ValType> &mt)
{
	//return TVector<TVector<ValType>>::operator +(mt);
	if (Size != mt.Size)
		throw invalid_argument("ADD SIZE NOT EQUAL");
	TMatrix res(*this);
	for (int i = 0; i<Size; i++)
...
TMatrix<ValType> TMatrix<ValType>::operator-(const TMatrix<ValType> &mt)
{
	//return TVector<TVector<ValType>>::operator -(mt);
	if (Size != mt.Size)
		throw invalid_argument("SUB SIZE NOT EQUAL");
	TMatrix res(*this);
	for (int i = 0; i<Size; i++)
	for (int j = i; j <Size; j++)
		res[i][j] = res[i][j] - mt.pVector[i][j];
